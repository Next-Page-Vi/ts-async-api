...
def test_move_with_invoker() -> None:
    ...
    ) == ClientMovedByOtherEvent(ctid=114, reasonid=1, clid=514, invoker=InvokerInfo(id=1919, name="aaa", uid="bbb"))
...
def test_kick() -> None:
    assert EventManager.parse_event(
        ...
        ctid=114, reasonid=4, clid=514, reasonmsg="wtf", invoker=InvokerInfo(id=1919, name="aaa", uid="bbb")
    )
...
def test_kick_no_msg() -> None:
    assert EventManager.parse_event(
        ...
        ctid=114, reasonid=4, clid=514, reasonmsg=None, invoker=InvokerInfo(id=1919, name="aaa", uid="bbb")
    )
